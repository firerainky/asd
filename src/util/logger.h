...
namespace zhejiangfhe {
    ...
    private:
        ...
        void Init() {
            ...
            std::string pattern =
                    "[%Y-%m-%d %H:%M:%S.%e] [Process:%P] [Thread:%t] [%^%l%$] <%s>|<%#>|<%!> %v";
            ...
            zj_logger =
                    std::make_shared<spdlog::logger>("zj_logger", begin(sinkList), end(sinkList));
            zj_logger->flush_on(
                    spdlog::level::info);// 设置当触发 info 或更严重的错误时立刻刷新日志到 disk
...
#define ZJTrace(...)                                                                               \
    BASE_LOGGER_CALL(Logger::GetInstance().GetLogger(), spdlog::level::trace, __VA_ARGS__)
#define ZJDebug(...)                                                                               \
    BASE_LOGGER_CALL(Logger::GetInstance().GetLogger(), spdlog::level::debug, __VA_ARGS__)
#define ZJInfo(...)                                                                                \
    BASE_LOGGER_CALL(Logger::GetInstance().GetLogger(), spdlog::level::info, __VA_ARGS__)
#define ZJWarn(...)                                                                                \
    BASE_LOGGER_CALL(Logger::GetInstance().GetLogger(), spdlog::level::warn, __VA_ARGS__)
#define ZJError(...)                                                                               \
    BASE_LOGGER_CALL(Logger::GetInstance().GetLogger(), spdlog::level::err, __VA_ARGS__)
#define ZJCritical(...)                                                                            \
    BASE_LOGGER_CALL(Logger::GetInstance().GetLogger(), spdlog::level::critical, __VA_ARGS__)
