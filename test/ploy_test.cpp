...
namespace zhejiangfhe {
    TEST(PolyTest, ConstructCopyAndAssignment) {
        ...
        std::shared_ptr<Params> params =
                ElemParamFactory::GenElemParams<Params>(m, bits);

        BPoly poly = BPoly(params);
        poly = {"1", "2", "0", "1"};

        EXPECT_EQ(poly.GetLength(), m / 2)
                << "Polynomial init with string list failed";

        BPoly poly2 = BPoly(params);
        poly2 = {1, 2, 0, 1};
        EXPECT_EQ(poly.GetLength(), m / 2)
                << "Polynomial init with int list failed";

        EXPECT_EQ(poly, poly2)
                << "Polynomials set with string list and int list should be same.";
        ...
        BPoly poly4 = poly;
        EXPECT_EQ(poly4, poly2)
                << "The new assignment polynomial should be equal to polynomial with "
    ...
    TEST(PolyTest, AddingTwoPolynomials) {
        ...
        EXPECT_EQ(computedPoly, expectedPoly)
                << "Addition operation on polynomials failed.";

        poly1.AddEq(poly2);
        EXPECT_EQ(poly1, expectedPoly)
                << "Addition operation in place on polynomials failed.";
    }
    ...
    TEST(PolyTest, SubtractPolynomial) {
        ...
        EXPECT_EQ(computedPoly, expectedPoly)
                << "Subtraction operation on polynomials failed.";

        poly1.SubEq(poly2);
        EXPECT_EQ(poly1, expectedPoly)
                << "Subtraction operation in place on polynomials failed.";
    }
    ...
    TEST(PolyTest, MultiplyScalar) {
        ...
        EXPECT_EQ(computedPoly, expectedPoly)
                << "MultiplyScalar operation on polynomials failed.";

        poly.MultiplyScalarEq(2);
        EXPECT_EQ(poly, expectedPoly)
                << "MultiplyScalar operation in place on polynomials failed.";
    }
    ...
    TEST(PolyTest, MultiplyForEvaluation) {
        ...
        EXPECT_EQ(poly1, expectedPoly)
                << "MultiplyForEvaluation operation in place on polynomials failed.";
    }
    ...
    TEST(PolyTest, SetFormat) {
        ...
        EXPECT_EQ(tempPoly, evalPoly)
                << "Convert values representation from coefficient format to "
                   "evaluation format failed.";

        tempPoly.SetFormat(Format::COEFFICIENT);
        EXPECT_EQ(tempPoly, coeffPoly)
                << "Convert values representation from evaluation format to "
    ...
    TEST(PolyTest, SwitchFormat) {
        ...
        std::shared_ptr<Params> params =
                ElemParamFactory::GenElemParams<Params>(m, nBits);
        ZJ_DEBUG("modulus: " << params->GetModulus()
                             << ", root: " << params->GetRootOfUnity()
        ...
        BPoly evalPoly(params, Format::EVALUATION);
        evalPoly =
                {1877267, 1022026, 3006168, 1772286, 1331622, 1039762, 869706, 1667670};

        BPoly poly(coeffPoly);

        poly.SwitchFormat();
        EXPECT_EQ(poly, evalPoly)
                << "Switch format from coefficient format to evaluation format failed";
        ZJ_DEBUG("Evalation poly: " << poly);

        poly.SwitchFormat();
        EXPECT_EQ(poly, coeffPoly)
                << "Switch format from evaluation format to coefficient format failed";
